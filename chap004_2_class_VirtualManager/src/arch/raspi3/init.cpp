...
//		l2[i].S0.AP=0b00;//RW, non EL0
	...
	virtman.init();
	virtman.enableMMU(
			pointerInc(reinterpret_cast<void*>(init_mmu_enabled), virtman.highMemBase()),
			pointerInc(reinterpret_cast<void*>(__stack_top), virtman.highMemBase())
	);
}
...
void  init_mmu_enabled()
{
	...
	vbar.Addr += virtman.highMemBase();
	vbar.write();

	kout << "in init_mmu_enabled\n";
