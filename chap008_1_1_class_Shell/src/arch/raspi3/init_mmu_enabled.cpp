...
void  init_mmu_enabled(size_t addressBits,void* lowFreeRamStart)
{
	...
	kout << INFO << "init virtual filesystem.\n";
	auto kernlog = new RAMVirtualFile("kern.log",FileType::F_FILE);
	vfs.addRootFile(kernlog);


	Vector<String> * args = new Vector<String>;
	args->emplaceBack("shell");
	VectorRef<String> argsRef(*args);
	Pid pid = svc_call<SvcFunc::createShell>(reinterpret_cast<size_t>(&argsRef),0 /*前台进程*/);
	delete args;

	kout << "created shell pid = " << pid << ".\n";
	...
	// 将系统日志重定向到文件
	auto kernlogWriter = new RAMFileCharacterWriter(kernlog);
	kout.redirect(kernlogWriter);
