...
FileType VirtualFile::type()const
{
	return F_DIRECTORY;
}
SubFileSystemType VirtualFile::subFSType()const
{
	return FS_ROOTFS;
}
...
VirtualFile*   VirtualFile::removeFile(VirtualFile * subFile)
{
	auto prev=subFile->_previousFile;
	auto next=subFile->_nextFile;
	// clear all p's fields
	subFile->_parent=nullptr;
	subFile->_nextFile=nullptr;
	subFile->_previousFile=nullptr;
	// change this,prev and next
	if(this->_subFile==subFile)
		this->_subFile=next;
	if(prev)
		prev->_nextFile = next;
	if(next)
...
VirtualFile*  VirtualFile::findFile(const VectorRef<String> &path)
{
	size_t i=0;
	auto p = this;
	if(path.size()>0 && path[0]=="/") // 表示切换到root
	{
		while(p->_parent)
			p = p->_parent;
		++i;
