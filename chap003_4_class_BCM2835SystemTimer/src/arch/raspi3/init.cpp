...
void init(uint64_t  el)
{
	...
	new (&excpHandler) ExceptionHandler<InterruptManager>(&intMan);
	...
	size_t  tick=1000000; //1000000us=1s
	size_t  timer = 1; // 经过测试,timer=0不会产生中断
	// 清除中断状态
	sysTimer.clearIntFlags(timer);
	sysTimer.addCompareValueUS(timer, tick);

	// 每1000个CPU时钟查询一次是否设置了中断控制信号
	while(!sysTimer.intFlagSet(timer))
	{
		kout << "timer not set.\n";
		delayCPU(1000,1000);
	}
	...
	while(n--)
	{
		kout << Hex(sysTimer.counterValue()) << "\n";
		delayCPU(1000,1000);
	}
}
