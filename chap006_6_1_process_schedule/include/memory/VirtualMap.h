...
// 文件作用域
namespace {
	template <class Descriptor=Descriptor4KBL0>
	struct LevelOfDescriptor{ enum {value=0};};
	template <>
	struct LevelOfDescriptor<Descriptor4KBL1>{ enum {value=1};};
	template <>
	struct LevelOfDescriptor<Descriptor4KBL2>{ enum {value=2};};
	template <>
	struct LevelOfDescriptor<Descriptor4KBL3>{ enum {value=3};};
};
...
public:
	template <class Descriptor>
	static constexpr size_t levelOfDescriptor()
	{
		return LevelOfDescriptor<Descriptor>::value;
	}
